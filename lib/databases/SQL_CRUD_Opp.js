...
/**
 * create multiple records from an entity
 * @param {*} entity
 * @param {*} res
 * @param {*} payload
 */
module.exports.createMulti = async (entity, payload) => {
  return new Promise(async (resolve, reject) => {
    try {
      const timestamp = new Date();

      const inserts = payload.map(e => {
        return {
          ...e,
          created_at: timestamp,
          updated_at: timestamp,
      ...
      // Run  creation process
      const process00 = await entity.bulkCreate(inserts);
